...
function playChildUI() {
	...
	function onPlayMusic(e) {
		console.log("播放音乐");
		SoundManager.playSound("http://119.23.231.250/count-down.mp3", 1, new Handler(this, onComplete));
	}
	...
	var text_money = this.text_money;
	var text_money_tip = this.text_money_tip;
	var text_height_tip = this.text_height_tip;
	...
	text_money._zOrder = 999;
	text_money_tip._zOrder = 999;
	box_bg._zOrder = 888;
	...
	function descending(obj) {
		...
		Tween.to(flySp, { y: flySp.y - oHeight }, 5000);
		Tween.to(obj, { height: 16 }, 5000, null, Handler.create(this, descending_rump, descendingRumpParam), 0, false);
		Tween.to(image_spray, { y: image_spray.y - oHeight }, 5000);
	}
	...
	function descending_rump(param) {
		Tween.to(param.obj, { height: 0 }, 1000);
		Tween.to(param.obj.light, { height: 0 }, 1000, null, Handler.create(this, function () {
			console.log('爆炸了');
			// 清除屏幕点击事件
		...
		// Tween.to(flySp, { y: flySp.y - 16 }, 1000);
	}
	...
	function imageRocketRemove() {
		...
		if (image_rocket_arr.length - 1 > 0) {
			...
			Tween.to(box_bg, { y: totalMove }, speed * surplus * (1 / ((6 - image_rocket_arr.length) + 1)), null, Handler.create(this, endFly), 0, true);
			...
			Tween.to(box_rocket, { scaleX: 1, scaleY: 1 }, 100, null, Handler.create(this, function () {
				Tween.to(image_rocket_head, { y: image_rocket_head.y - 400 , rotation: 180 }, 2000, Ease.quartOut, Handler.create(this, function () {
					image_parachute.pos(box_rocket.x, box_rocket.y + image_rocket_head.y);
					image_parachute.visible = true;
					Tween.to(image_parachute, { y: image_parachute.y + Laya.stage.height}, 3000, Ease.sineIn);
					Tween.to(image_rocket_head, { y: image_rocket_head.y + Laya.stage.height }, 3000, Ease.sineIn);
				}), 0, false);
	...
	function runRocket(param) {
		...
		var surplus = totalMove - box_bg.y;
		Tween.to(box_bg, { y: totalMove }, speed * surplus * 1 / 1.1, null, Handler.create(this, endFly), 0, true);
